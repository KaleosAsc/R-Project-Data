...
La influencia del internet ha cambiado la dinámicas de la relevancia de un servicio, las personas denominados en un rol de usuarios buscan la buena imagen a la hora descargar algún tipo de aplicación por medio de las reseñas que validan la experiencia del acceso a este mismo para tomar una decisión, en este caso las reseñas positivas pueden influir en prestigio asociado al negocio.

Por eso, se pueden dar sponsors de "opinión spamming" donde se patrocinan reseñas fraudulentas para promover aplicaciones o en caso de competencia desleal, para desvalorar una aplicación de otro negocio por el área de un servicio.
...
Generar un sistema de minería de datos que aplica el algoritmo de Naive Bayes para determinar y/o detectar reseñas spam multilenguaje para aplicaciones.
...
Aquí tienes el texto con solo correcciones ortográficas y sin modificar el contenido original ni la redacción:
...
* **rating**: Puntuación de la aplicación para valoración si presenta algún tipo de inflación.
...
Dentro del marco de categorización de variables como la agregada **spam_flag**, se convirtió a factor entre las dos opciones que hay de "ham" y "spam", de tipo label encoding, lo que nos permite clasificar el estado de autenticidad para el entrenamiento del modelo.

Además, se aplicó técnicas para el mismo modelo la enumeración de frecuencia de las palabras de cada una de las reseñas, para después pasar a una codificación binaria dentro de los parámetros de validez (One-Hot Encoding).
...
En este caso del desarrollo, se escogió una técnica de clasificación, por medio del algoritmo de Naive Bayes, siendo machine learning de carácter supervisado para la tarea de clasificación de textos (en este caso aplicado a textos spam), basado en el teorema de Bayes que calcula la probabilidad condicional de que una instancia pertenezca a una clase.

Además, dentro del algoritmo se aplicó Vcorpus donde se permitió aplicar procesos de refinamiento al modelo, donde se pasó todo el texto a minúsculas, quitó puntuaciones, números y *stopwords* en inglés, con una matriz para generar una representación numérica de la frecuencia de las palabras con reducción de términos raros.

Con esto se dividió el modelo entre un 70% de entrenamiento y el 30% de prueba, donde se usó una binarización de términos a la matriz de frecuencia, para finalizar con el entrenamiento del modelo con la finalidad de tener un algoritmo con alta refinación y respuesta de predicción.

Para este caso, este tipo de algoritmo nos da una predicción acertada para textos de manera categorizada, además de que tiene una tolerancia al ruido, adecuado a problemas reales (efectivo en nuestro *dataset* que presenta datos reales con alta variabilidad por ser multilenguaje), además, por su fácil cálculo de probabilidad, nos permite una fácil interpretación de los datos.
...
-1 Preprocesar y transformar los datos textuales, incluyendo la traducción, limpieza y normalización del texto, con el fin de obtener un corpus homogéneo y adecuado para el análisis.
-2 Construir y aplicar una bolsa de palabras (Bag of Words) que permita identificar patrones lingüísticos y características relevantes entre reseñas legítimas y reseñas spam.
-3 Entrenar y evaluar el modelo Naive Bayes utilizando métricas como Accuracy, F1-Score y la matriz de confusión, para validar su capacidad predictiva
-4 Analizar los resultados obtenidos y validar si el modelo cumple con las hipótesis planteadas, determinando su eficacia para la detección automatizada de reseñas falsas.


##Hipotesis
H1: Las aplicaciones con mayor presencia digital presentan más reseñas spam
A partir de las variables **app_name**, es posible observar que las aplicaciónes con mayor número total de reseñas, tambien presentan una proporción más alta de reseñas clasificadas como spam. 
Esto podemos decir, que las apps más populares o visibles son más susceptibles a manipulación de reseñas o comentarios automatizados. 

**Hipótesis, confirmada** 

H2: Las aplicaciones en el sistema operativo Windows concentran más reseñas spam que otros dispositivos.

utilizando la variable **device_type**, se puede analizar la distribución de **spam_flag** por dispositivo. 
Aunque existen casos de spam en todos los dispositivos, no se observó una diferencia estadísticamente significativa entre plataformas(por ejemplo, Android, iOS o Windows). 

**hipótesis parcialmante confirmada**

H3: Las aplicaciones con puntuaciones extremas (muy bajas o muy altas) tienden a tener más reseñas spam.

la variable rating, muestra que las reseñas con califaciónes de 1 a 5 estrellas presentan una mayor proporción de mensajes marcados como spam. 
**Hipótesis confirmada**
...
El proceso nos permitió analizar los diferentes factores que influencian a los modelos de *machine learning* supervisado, donde se destaca que dentro del proceso de manejo del *dataset*, a pesar de que se aplicó un proceso de limpieza de datos (valores nulos, imputación de datos, corrección de datos incorrectos), se debieron generar unos criterios de evaluación para categorizar las reseñas.

Debido a esto, el equipo de trabajo llegó a la estimación de que el 90% del *dataset* original proporcionado en el marco de la clase era completamente *spam* (reseñas repetidas, traducciones que no coincidían con una reseña verdadera, datos de tipo *lorem ipsum*).

En solución de ese aspecto, se optó por la inserción de nuevos datos de reseñas verificados por el equipo que simulaban el comportamiento de una reseña verdadera.

Adicional, se evaluó con el equipo que, ante el aumento de criterios del comportamiento del *dataset*, decrecía la calidad de predicción de los datos. Por lo tanto, se optó por manejar solo el criterio de la bolsa de palabras.

Por otra parte, el factor multilenguaje complicaba los aspectos de clasificación supervisada del equipo, por eso se decidió montar un sistema de traducción *cloud* (Azure), que nos permitió traducir las reseñas a un lenguaje común para la aplicación de la bolsa de palabras y la veracidad de la clasificación.

Por último, a pesar de haber ya aplicado procesos de selección, limpieza y transformación de datos, el mismo modelo de Naive Bayes tiene procesos de limpieza y categorización numérica para refinar los procesos de predicción de los datos.
...
prediction_fun <- function(texto) {
  # Crear un corpus con el nuevo texto
  ...
  #  Binarizar (igual que hiciste con train/test)
  ...
  #  Alinear columnas por si falta alguna palabra
